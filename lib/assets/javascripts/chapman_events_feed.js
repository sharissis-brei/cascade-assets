...
(function( $ ){
  ...
  ChapmanEventsFeed = function(options) {
    ...
    this.title        = options.title || 'Upcoming Events';
  ...
  ChapmanEventsFeed.prototype.onFeedSuccess = function(data) {
    this.$element.append('<div class="feed-column"> \
                            <h3 class="feed-title">'+this.title+'</h3>' +
                              data +
  ...
  /**
   * The bacllback for when feed ajax encountered an error;
   */
  ...
  ChapmanEventsFeed.prototype.onFeaturedSuccess = function(data) {
    this.$element.prepend('<div class="featured-column"> \
                            <h3 class="feed-title">Featured Event</h3>' +
                            data +
                          '</div>');
  };
