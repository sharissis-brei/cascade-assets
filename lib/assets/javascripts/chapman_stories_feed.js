...
;(function($, window, document, undefined) {
  ...
  var defaults = {
    baseUrl: 'https://social.chapman.edu/',
    storiesPerPage: 4,
  ...
  $.extend(Plugin.prototype, {
    init: function() {
      ...
      this.feedUrl          = this.$element.data('chapmanStoriesFeedUrl') || this.settings.dataFeedUrl;
      this.topStoryUrl      = this.$element.data('chapmanStoriesTopStoryUrl') || this.settings.dataTopStoryUrl;
    ...
    buildTopStoryElement: function() {
      ...
      var params = {
        url: this.topStoryUrl
      };

      // This will return HTML for top story.
      $.ajax({
        method: 'GET',
        url: this.buildSocialApiEndpoint('top_story'),
        data: params,
    ...
    buildStoriesFeedElement: function() {
      ...
      $.ajax({
        method: 'GET',
        url: this.buildSocialApiEndpoint('feed'),
        data: params,
        success: function(data) {
          if ( data.length ) {
    ...
    buildSocialApiEndpoint: function(path) {
